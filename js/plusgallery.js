  ...
  $.fn.plusGallery = function(options){
    ...
    var pg = {
      ...
      loadSingleAlbum:function(){
        ...
        {
          case 'google':
            url = 'https://picasaweb.google.com/data/feed/base/user/' + pg.userId + '/albumid/' + pg.albumId + '?alt=json&hl=en_US&kind=photo&authkey='+pg.apiKey;
          break;
          case 'flickr':
              ...
              if(i < pg.limit) {
                ...
                switch(pg.type)
                {
                  ...
                  if (imgSrc.match('gif') ){
                    ...
                    isMovie = true;
                    vidObj = obj.media$group.media$content[2];
                    pg.imgTotal--;
                  }
                ...
                if(!isMovie){
                  ...
                  var html = '<li class="pgthumb">';
                  //html += '<a href="' + vidObj.url + '">';
                  html += '<video controls width='+vidObj.width+' height='+vidObj.height+' id=pgthumbimg' + i + '" class="pgthumbimg" alt="' + imgTitle + '" title="' + imgTitle + '"' + imgBg + '>'
                  html += '<source src=' + vidObj.url + '/></video>';
                  //html += '</a>'
                  html += '</li>';
                  $('#pgthumbs').append(html);
                }
